...
export const demoNetworkService: NetworkService = { async authorizeDevice() { return serviceSuccess({ authorized: true }) }, async revokeDevice() { return serviceSuccess({ revoked: true }) }, async getDeviceStatus() { return serviceSuccess("AUTHORIZED") }, async disconnectSession() { return serviceSuccess({ disconnected: true }) }, async getNetworkStatus() { return serviceSuccess(nodes.map((node, index) => ({ id: `node-${index + 1}`, createdAt: "2026-09-01T00:00:00.000Z", updatedAt: "2026-09-16T00:00:00.000Z", name: node.name, type: node.type, status: node.status, latencyMs: Number.parseInt(node.latency), clientCount: Number(node.clients) }))) } }
